...
LEARNING_RATE=1e-4               # 回退：5e-4 顶发散，回稳定 1e-4
LR_DIT=1e-5
WEIGHT_DECAY=0.01
GRADIENT_ACCUMULATION_STEPS=1    # 保留：8→1，更新次数×8（唯一安全的加速器，零额外计算）
MAX_GRAD_NORM=1.0                # 回退：5.0 撤防致 attn_norm 爆 40985，回 1.0 保护
...
TRAIN_SCRIPT="${PROJECT_ROOT}/src/pipeline/v4/train_v4_stage1_dual.py"
...
echo "  TRAIN_HIGH          : ${TRAIN_HIGH}  (0=只训 low)"
...
# 验证阶段 TRAIN_HIGH=0 时整段跳过（只训 low 即可证明机制，省一半时间）
# ============================================================
if [ "${TRAIN_HIGH}" != "1" ]; then
    echo ""
    echo "===== 跳过 high_noise_model（TRAIN_HIGH=0，验证阶段只训 low）====="
    echo "===== 训练完成：low_noise_model → ${OUTPUT_DIR}/low_noise_model/ ====="
    exit 0
fi
