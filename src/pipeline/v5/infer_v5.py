...
两种数据入口（二选一，additive；不设 episode 三参时 --image/--action_path
逐字节与改前一致）：
  A. action_path 模式（原口径，严格对齐 v4 infer_v4.py）：
     - --image           首帧图像（PNG/JPG）
     - --action_path     目录，含 poses.npy[T,4,4] / action.npy / intrinsics.npy
                         （整条 GT 动作轨迹，是控制信号；模型据此渲染视频）
  B. episode 模式（demo 用：直接取重访数据集整条 episode，省手动拼 action）：
     - --episode_id      数据集 episode id；特殊值 "first"/"top" = 取 metadata
                         CSV 按 revisit_quality 排序后的第一个（最高质量重访 ep）
     - --dataset_dir     含 metadata CSV + clips/ 的数据集根（episode 模式必填）
     - --metadata        相对 dataset_dir 的 CSV（默认 metadata_verify_train.csv）
     内部走 retrieval_probe.load_episode_clips → build_episode_data →
     _decode_episode_video：首帧 = frames[0]；poses/actions/intrinsics 直接取
     ep.poses[T,4,4]/ep.actions[T,4]/ep.intrinsics[T,4]，不落 poses.npy，主循环
     按 [i*frame_num:(i+1)*frame_num] 逐 clip 切片。num_clips 上限 = T//frame_num。
...
def _parse_args():
    ...
    # ---- 数据（二选一：action_path 模式 或 episode 模式）----
    # action_path 模式（原口径，严格对齐 v4 infer_v4.py）
    p.add_argument("--image", type=str, default=None,
                   help="首帧图像路径（PNG/JPG）。episode 模式下忽略，首帧取 frames[0]。")
    p.add_argument("--action_path", type=str, default=None,
                   help="目录，含 poses.npy[T,4,4] / action.npy / intrinsics.npy"
                        "（整条 GT 动作轨迹，是控制信号）。episode 模式下忽略。")
    # episode 模式（demo 用：直接取重访数据集整条 episode）
    p.add_argument("--episode_id", type=str, default=None,
                   help="数据集 episode id；开启 episode 模式。特殊值 'first'/'top' = "
                        "取 metadata CSV 按 revisit_quality 排序后的第一个 episode"
                        "（最高质量重访 ep，最适合 demo）。设此参数时 --image/--action_path "
                        "被忽略，--dataset_dir 必填。")
    p.add_argument("--dataset_dir", type=str, default=None,
                   help="含 metadata CSV + clips/ 的数据集根目录（episode 模式必填）。")
...
def _load_action_inputs(args, height: int, width: int, run_dir):
    """加载整条动作轨迹 + 首帧，返回两模式共用的 (poses/actions/intrinsics, first_img)。

    两种模式（additive，互斥）：
      A. action_path 模式（--episode_id 未设）：np.load poses/action/intrinsics.npy +
         Image.open(--image)。逐字节对齐改前口径。
      B. episode 模式（--episode_id 设）：走 retrieval_probe.load_episode_clips →
         build_episode_data → _decode_episode_video。首帧 = frames[0]（转 PIL）；
         poses/actions/intrinsics 直接取 ep.poses[T,4,4]/ep.actions[T,4]/
         ep.intrinsics[T,4]（不落 poses.npy，主循环按 clip 切片）。num_clips 上限 =
         T // frame_num（向下取整后覆盖到 args.num_clips）。

    Args:
        args:   parsed argparse（读 episode_id/dataset_dir/metadata/image/action_path/
                frame_num/num_clips）。
        height/width: 生成分辨率（episode 模式 _decode_episode_video 用）。
        run_dir: infer 产出目录（episode 模式另存 first_frame.png 供对照）。

    Returns:
        poses_np:     [T,4,4] np.float32（绝对 c2w）。
        actions_np:   [T,4]  np.float32。
        intrinsics_np:[T,4]  np.float32。
        first_img:    PIL.Image（首帧，作为 clip 0 的 generate img 入口）。
        episode_id:   str（episode 模式=实际 ep_id；action_path 模式="<action_path>"）。
    """
    if args.episode_id is not None:
        # ---- episode 模式 ----
        if not args.dataset_dir:
            raise ValueError(
                "episode 模式（--episode_id 设）要求 --dataset_dir 非空"
                "（含 metadata CSV + clips/ 的数据集根）。"
            )
        # 复用 eval_v5 同源 import（load_episode_clips / build_episode_data /
        # _decode_episode_video 来自 pipeline.eval.retrieval_probe）。
        from pipeline.eval.retrieval_probe import (
            load_episode_clips,
            build_episode_data,
            _decode_episode_video,
        )

        # first/top = metadata CSV 按 revisit_quality 排序后的第一个 ep（最高质量重访）
        ep_filter = None if args.episode_id in ("first", "top") else [args.episode_id]
        ...
        if not ep_ids:
            raise RuntimeError(
                f"episode 模式：metadata {args.metadata} 在 {args.dataset_dir} 内"
                f"未返回任何 episode（filter={ep_filter}）。"
            )
        ep_id = ep_ids[0]
        logger.info(
            "episode 模式：选中 episode_id=%s （请求=%s，CSV 命中 %d 个 ep）",
            ep_id, args.episode_id, len(ep_ids),
        )

        ep = build_episode_data(ep_id, ep_groups[ep_id], clip_overlap_frames=0)
        if ep is None:
            raise RuntimeError(
                f"episode 模式：build_episode_data 对 ep={ep_id} 返回 None"
                "（全部 clip 重要文件缺失？）。"
            )

        poses_np = ep.poses.astype(np.float32)           # [T,4,4]
        actions_np = ep.actions.astype(np.float32)       # [T,4]
        intrinsics_np = ep.intrinsics.astype(np.float32) # [T,4]
        T = poses_np.shape[0]

        # num_clips 上限 = T // frame_num（不足取 floor；log 实际 num_clips）
        max_clips = max(1, T // args.frame_num)
        if args.num_clips > max_clips:
            logger.warning(
                "episode %s: T=%d, frame_num=%d → num_clips %d > 上限 %d，"
                "向下裁到 %d（剩余尾帧 %d 不生成）。",
                ep_id, T, args.frame_num, args.num_clips, max_clips, max_clips,
                T - max_clips * args.frame_num,
            )
            args.num_clips = max_clips

        # 解码全 episode 视频 → 首帧（[3,H,W] float [-1,1] → PIL）
        frames = _decode_episode_video(ep, height=height, width=width)  # [T,3,H,W]
        first_frame_chw = frames[0]  # [3,H,W] float [-1,1]
        first_hwc = (first_frame_chw.transpose(1, 2, 0) * 127.5 + 127.5
                     ).clip(0, 255).astype(np.uint8)  # [H,W,3] uint8
        first_img = Image.fromarray(first_hwc)

        # 另存 first_frame.png 供对照
        try:
            first_png = os.path.join(str(run_dir), "first_frame.png")
            first_img.save(first_png)
            logger.info("episode 首帧另存 → %s", first_png)
        except Exception as exc:  # noqa: BLE001
            logger.warning("episode 首帧另存失败（非致命）: %s", exc)

        logger.info(
            "episode 模式 loaded: ep=%s T=%d num_clips=%d poses=%s actions=%s "
            "intrinsics=%s",
            ep_id, T, args.num_clips, poses_np.shape, actions_np.shape,
            intrinsics_np.shape,
        )
        return poses_np, actions_np, intrinsics_np, first_img, ep_id

    # ---- action_path 模式（原口径，逐字节对齐改前）----
    if not (args.action_path and os.path.isdir(args.action_path)):
        logger.error("action_path 不存在或非目录：%s；退出。", args.action_path)
        raise SystemExit(0)
    if not args.image:
        logger.error("action_path 模式要求 --image（首帧图像路径）；退出。")
        raise SystemExit(0)
    try:
    ...
    return poses_np, actions_np, intrinsics_np, first_img, str(args.action_path)
...
def main():
    ...
    # ---- 加载整条动作轨迹 + 首帧（action_path 模式 / episode 模式 二选一）----
    poses_np, actions_np, intrinsics_np, first_frame_img, source_id = \
        _load_action_inputs(args, height=height, width=width, run_dir=run_dir)
    ...
    current_img = first_frame_img  # clip 0 的 generate img 入口
