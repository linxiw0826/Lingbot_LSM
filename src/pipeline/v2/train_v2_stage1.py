"""
train_v2.py — LingBot-World Memory Enhancement 训练脚本 v2
============================================================
核心原则：完全对齐 csgo-finetune-v2（refs/csgo-finetune-v2/train_lingbot_csgo.py），
          在此基础上加入 Memory Enhancement（WanModelWithMemory + NFPHead）。

与 v2 的偏差（仅 Memory Enhancement 相关）：
  1. 模型加载：WanModel → WanModelWithMemory.from_wan_model()
  2. Stage1/Stage2 冻结策略（freeze_for_stage）
  3. NFP Loss（forward hook 注册在 model.blocks[-1]）
  4. training_step 新增 memory_states=None 和 NFP Loss 计算
  5. 命令行新增 --stage / --nfp_loss_weight / --lr_dit
  6. LoRA target_modules 额外包含 memory_cross_attn.{q,k,v,o}

Usage:
    accelerate launch --config_file accelerate_config_zero2.yaml \\
        train_v2.py \\
        --ckpt_dir /models/lingbot-world-base-act/ \\
        --dataset_dir /data/csgo_processed/ \\
